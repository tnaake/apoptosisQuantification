...
hallmark_genes <- tibble(protein = hallmark_genes)
```
...
We then calculate the loadings of markers and non-markers. We wil look here 
at loadings of principal component 1.

```{r calculate_loadings, eval = TRUE, echo = TRUE}
## loadings of markers
loadings_markers <- getLoadingsOfMarkers(prot = prot, 
    markers = markers_apoptosis)
vals_markers <- setNames(loadings_markers$PC1, loadings_markers$protein)

## loadings of non-markers 
vals_all <- prcomp(t(prot))$rotation[, 1]

## combine the values and create a tibble containing the loadings of 
## markers and non-markers
tbl <- createLoadingsTbl(vals_markers = vals_markers, vals_all = vals_all)
```

We then visualize the results (loadings of markers vs. loadings of non-markers).
...
prop_mito <- calculateProportionOfMitochondrialProteins(prot = prot_raw, 
    id = "Symbol")
```
...
`signatures` that contains the columns:
 ...
 - `protein`: name of marker proteins,
 - `fold_change`: associated negative or positive fold change associated with
   the marker.
