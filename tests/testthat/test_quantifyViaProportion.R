...
test_that("calculateProportionOfMitochondrialProteins works.", {
    ...
    prop <- calculateProportionOfMitochondrialProteins(prot = prot, 
        id = "Symbol")
    ...
    prop_foo <- calculateProportionOfMitochondrialProteins(prot = prot, 
        id = "Ensembl")
    expect_equal(length(prop), 10)
    expect_equal(names(prop), colnames(prot))
    expect_equal(as.vector(prop_foo), rep(0, 10))
    expect_error(calculateProportionOfMitochondrialProteins(prot = 1:10),
        "non-character argument")
    mat_foo <- matrix(1:100, ncol = 10)
    expect_error(calculateProportionOfMitochondrialProteins(prot = mat_foo),
        "non-character argument")
    expect_error(calculateProportionOfMitochondrialProteins(prot = prot, 
